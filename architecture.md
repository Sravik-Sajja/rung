...
All request bodies are validated with shared schemas. Production identity and role come from the authenticated session, not a client-provided `studentId` or `classId`; routes also enforce RLS-compatible membership checks. `DEMO_MODE` is explicitly environment-gated and production-disabled unless the fictional hosted demo also sets `ALLOW_DEMO_IN_PROD=true`. In that narrow demo mode it resolves a server-created temporary participant, a joined workspace learner, or both, each bound by its own opaque httpOnly cookie; a client-supplied ID never selects an identity on any path.

...
export function PracticeFlow({ sessionId }: { sessionId: string }) {
  ...
  if (!data || !currentItem) return <><p>{message ?? "Practice is complete."}</p><Link href="/student/mastery" className="mt-5 inline-block text-indigo-700 underline">View your skill status</Link></>;
  ...
  return <><p className="text-sm text-slate-600">{data.progress.completedItemCount} of {data.progress.totalItemCount} completed</p><p className="mt-3 rounded bg-slate-50 p-4 text-lg">{currentItem.prompt}</p><div className="mt-4"><AnswerInput onSubmit={submitAnswer} /></div>{message && <p className="mt-3 text-sm text-slate-700" role="status">{message}</p>}<div className="mt-5"><HintLadder activeLevel={hintLevel} hint={hint} onRequest={requestHint} /></div><div className="mt-5"><PeerGateCard approachUnlocked={currentPeerGate.approachUnlocked} fullSolutionUnlocked={currentPeerGate.fullSolutionUnlocked} onSubmitAttempt={submitPeerAttempt} />{peerSolution?.peerSolution && <div className="mt-3 rounded-lg bg-slate-50 p-4"><p className="font-medium">{peerSolution.peerSolution.authorAlias}’s approach</p><p className="mt-1 text-sm">{peerSolution.peerSolution.approachText}</p>{peerSolution.peerSolution.fullSolution && <p className="mt-2 text-sm"><span className="font-medium">Complete solution:</span> {peerSolution.peerSolution.fullSolution}</p>}</div>}</div></>;
}

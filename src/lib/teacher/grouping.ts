...
import type { MasteryRecord, Subskill, TeacherDashboard, TeacherGroup } from "@/lib/types";
import { DEMO_CLASS_ID, demoGroupPlans, demoMastery, demoStudents, demoSubskills } from "@/lib/demo-data";

const MINIMUM_GROUP_SIZE = 2;

export function groupStudentsByNeed(
  mastery: MasteryRecord[],
  subskills: Subskill[] = demoSubskills,
  minimumSize = MINIMUM_GROUP_SIZE,
): TeacherGroup[] {
  ...
  return subskills.flatMap((subskill) => {
...
export function getDemoTeacherDashboard(classId = DEMO_CLASS_ID): TeacherDashboard | null {
  ...
  return { classId, students: demoStudents, subskills: demoSubskills, cells: demoMastery, groups: groupStudentsByNeed(demoMastery, demoSubskills) };
}

...
export async function recordStudentResponse(input: { studentId: string; classId: string; itemId: string; answer: string; context: "diagnostic" | "practice" }) {
  ...
  const { data: prior } = await supabase.from("mastery").select("evidence_count").eq("student_id", input.studentId).eq("class_id", input.classId).eq("subskill_id", item.subskill_id).maybeSingle();
  ...
  const { error: masteryError } = await supabase.from("mastery").upsert({ student_id: input.studentId, class_id: input.classId, subskill_id: item.subskill_id, level, evidence_count: evidenceCount, evidence_summary: evidenceSummary, last_evaluated_at: new Date().toISOString() }, { onConflict: "student_id,class_id,subskill_id" });

...
type DiagnosticRun = { id: string; studentId: string; assignmentId: string; answers: Map<string, { answer: string; isCorrect: boolean }> };
...
const diagnostics = new Map<string, DiagnosticRun>();
const practices = new Map<string, PracticeRun>();
...
export function startDemoDiagnostic(studentId: string = canonicalDemoIds.mayaStudentId) {
  const run: DiagnosticRun = { id: id("diagnostic"), studentId, assignmentId: canonicalDemoIds.diagnosticAssignmentId, answers: new Map() };
  diagnostics.set(run.id, run);
  return {
...
export function recordDemoDiagnosticResponse(input: { diagnosticSessionId: string; studentId: string; itemId: string; answer: string }) {
  const run = diagnostics.get(input.diagnosticSessionId);
  ...
  run.answers.set(item.id, { answer: input.answer.trim(), isCorrect });
...
export function completeDemoDiagnostic(input: { diagnosticSessionId: string; studentId: string }) {
  ...
  const practice: PracticeRun = {
    ...
    items: selected.map((item, index) => ({ id: id("practice-item"), item, position: index + 1, status: "pending" })),
    mastery: new Map(),
  };
...
export function getDemoPractice(sessionId: string, studentId: string) {
  const run = practices.get(sessionId);
...
export function recordDemoPracticeResponse(input: { practiceSessionId: string; practiceSessionItemId: string; studentId: string; answer: string }) {
  const run = practices.get(input.practiceSessionId);
